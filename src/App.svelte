...
<svg bind:this={svg} viewBox="-500 -500 1000 1030" style="width:100%; height: auto;" width="500" height="500" on:dblclick={clickSetTarget} on:click={clickResetTarget}>
	...
	{#if movement}
	<path d="M{x},{y}h{(movement.left-movement.right)*radius*1.4}M{x},{y}v{(movement.top-movement.bottom)*radius*1.4}" stroke-width="15" stroke="orange" />
	{/if}
	
		<circle cx={x} cy={y} r="{radius*1}" fill="{color}" stroke="{color}a" stroke-width="2"  />
